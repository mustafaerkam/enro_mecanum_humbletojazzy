...
def audit(workspace: Path) -> dict[str, Any]:
    ...
    for name, profile in profiles.items():
        ...
        if world.is_file():
            _record(
                ...
                'gz::sim::systems::Imu' in world.read_text(),
                str(world),
            )
    ...
    _record(
        checks,
        'harmonic.friction_frame',
        xacro_text.count('gz:expressed_in="base_footprint"') == 1
        and 'gz:expressed_in="base_link"' not in xacro_text,
        'fdir1 must be expressed in base_footprint: URDF->SDF lumps the '
        'fixed-joint base_link into base_footprint, leaving base_link only '
        'as an SDF <frame>. dartsim needs a real link (body node) for '
        'setFirstFrictionDirectionFrame; an unresolvable frame silently '
        'falls back to the rotating wheel frame.',
    )
    _record(
        checks,
        'sensors.lidar_contract',
        '<update_rate>10</update_rate>' in xacro_text
        and '<samples>640</samples>' in xacro_text
        and '<min>0.12</min>' in xacro_text
        and '<max>12.0</max>' in xacro_text,
        'expected 10 Hz, 640 samples, 0.12..12.0 m',
    )
    ...
    sim_control = (src / 'mecanum_control/urdf/ros2_control_sim.xacro').read_text()
    nav_params_text = (nav / 'params/nav2_params.yaml').read_text()
    _record(
        checks,
        'command_chain.jazzy_stamped_reference',
        'mecanum_drive_controller/reference:=cmd_vel' in sim_control
        and nav_params_text.count('enable_stamped_cmd_vel: true') >= 4,
        'Jazzy command chain must remain TwistStamped end-to-end',
    )
    ...
    world_text = '\n'.join(
        (sim / 'worlds' / profile['world_file']).read_text() for profile in profiles.values()
    )
    _record(
        checks,
        'gazebo.harmonic_system_names',
        'ignition::gazebo' not in world_text and 'gz::sim::systems::Physics' in world_text,
        'world plugins must use Gazebo Harmonic names',
    )
