...
class MotionProbe(Node):
    ...
    def settle(self, seconds):
        ...
        s = self.hold(self.make_twist(), seconds, record=True)
        z = [p['gt']['z'] for p in s]
        return {
...
def run_case(node, name, vx, vy, wz, seconds, settle_s):
    node.hold(node.make_twist(), settle_s)             # once tam dur
    a = node.sample()
    tw = node.make_twist(vx, vy, wz)
    series = node.hold(tw, seconds, record=True)
    stop_series = node.hold(node.make_twist(), 2.0, record=True)
    b = series[-1]
...
def main():
    ...
    ap.add_argument(
        '--case', dest='selected_cases', action='append',
        choices=['A_ileri', 'B_geri', 'C_donus', 'D_yana'],
        help='Yalniz secilen testi calistir; taze simülasyonda izolasyon icin tekrarlanabilir',
    )
    args = ap.parse_args()
